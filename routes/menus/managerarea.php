...
Menu::register('managerarea.sidebar', function (MenuGenerator $menu) {
    $menu->findByTitleOrAdd(trans('cortex/foundation::common.user'), 20, 'fa fa-users', 'header', [], [], function (MenuItem $dropdown) {
        $dropdown->route(['managerarea.cortex.auth.managers.index'], trans('cortex/auth::tenantable.managers'), null, 'fa fa-user')->ifCan('list', app('cortex.auth.manager'))->activateOnRoute('managerarea.cortex.auth.managers');
